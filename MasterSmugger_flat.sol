...
contract GovernanceToken is ERC20, Ownable, Authorizable {
    ...
    /// @notice Creates `_amount` token to `_to`. Must only be called by the owner (MasterSmugger).
...
// File src/MasterSmugger.sol

// MasterSmugger is the master smugger of whatever smug the GovernanceToken represents.
//
// Note that it's ownable and the owner wields tremendous power. The ownership
// will be transferred to a governance smart contract once GovernanceToken is sufficiently
// distributed and the community can show to govern itself.
//
contract MasterSmugger_flat is Ownable, Authorizable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    modifier nonDuplicated(IERC20 _lpToken) {
        require(poolExistence[_lpToken] == false, "MasterSmugger::nonDuplicated: duplicated");
        _;
    }
    ...
    function add(
        ...
        require(poolId1[address(_lpToken)] == 0, "MasterSmugger::add: lp is already in pool");
        if (_withUpdate) {
            massUpdatePools();
        }
    ...
    // Deposit LP tokens to MasterSmugger for GovernanceToken allocation.
    function deposit(
        uint256 _pid,
        uint256 _amount,
        address _ref
    ) public nonReentrant {
        require(_amount > 0, "MasterSmugger::deposit: amount must be greater than 0");
    ...
    // Withdraw LP tokens from MasterSmugger.
    function withdraw(
        uint256 _pid,
        uint256 _amount,
        address _ref
        ...
        UserGlobalInfo storage refer = userGlobalInfo[_ref];
        UserGlobalInfo storage current = userGlobalInfo[msg.sender];
        require(user.amount >= _amount, "MasterSmugger::withdraw: not good");
        if (_ref != address(0)) {
            refer.referrals[msg.sender] = refer.referrals[msg.sender] - _amount;
            refer.globalRefAmount = refer.globalRefAmount - _amount;
        }
    ...
    function safeGovTokenTransfer(address _to, uint256 _amount) internal {
        ...
        require(transferSuccess, "MasterSmugger::safeGovTokenTransfer: transfer failed");
    }
