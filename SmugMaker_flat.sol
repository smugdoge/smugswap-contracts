...
// File @smugswap/core/contracts/interfaces/IUniswapV2Factory.sol@v2.0.3
...
// File @smugswap/core/contracts/interfaces/IUniswapV2Pair.sol@v2.0.3
...
// File @smugswap/core/contracts/interfaces/IUniswapV2ERC20.sol@v2.0.3
...
// T1 - T4: OK
contract SmugMaker_flat is Ownable {
    using SafeMath for uint256;
    ...
    // V1 - V5: OK
    address public immutable smugStaker;
    ...
    function setBridge(address token, address bridge) external onlyOwner {
        // Checks
        require(token != govToken && token != weth && token != bridge, "SmugMaker: Invalid bridge");
    ...
    modifier onlyEOA() {
        ...
        require(msg.sender == tx.origin, "SmugMaker: must use EOA");
        _;
    }

    // F1 - F10: OK
    // F3: _convert is separate to save gas by only checking the 'onlyEOA' modifier once in case of convertMultiple
    // F6: There is an exploit to add lots of GovernanceTokens to the SmugStaker, run convert, then remove the GovernanceTokens again.
    //     As the size of the SmugStaker has grown, this requires large amounts of funds and isn't super profitable anymore
    ...
    function _convert(address token0, address token1) internal {
        ...
        require(address(pair) != address(0), "SmugMaker: Invalid pair");
        // balanceOf: S1 - S4: OK
        // transfer: X1 - X5: OK
    ...
    function _convertStep(
        ...
        if (token0 == token1) {
            ...
            if (token0 == govToken) {
                IERC20(govToken).safeTransfer(smugStaker, amount);
                govTokenOut = amount;
            } else if (token0 == weth) {
            ...
            // eg. GovToken - ETH
            IERC20(govToken).safeTransfer(smugStaker, amount0);
            govTokenOut = _toGovToken(token1, amount1).add(amount0);
        } else if (token1 == govToken) {
            // eg. USDT - GovToken
            IERC20(govToken).safeTransfer(smugStaker, amount1);
    ...
    function _swap(
        ...
        require(address(pair) != address(0), "SmugMaker: Cannot convert");
    ...
    function _toGovToken(address token, uint256 amountIn) internal returns (uint256 amountOut) {
        // X1 - X5: OK
        amountOut = _swap(token, govToken, amountIn, smugStaker);
    }
}
